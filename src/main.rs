...
fn check_dependencies() -> Result<()> {
    for cmd in &["curl", "file"] {
        Command::new("which")
            .arg(cmd)
            .output()
            .with_context(|| format!("Required command not found: {}. Please install it first.", cmd))?;
    }
    Ok(())
}
...
fn convert_pdf(pdf_path: &Path, config: &Config, notif: &NotificationManager) -> Result<String> {
    ...
    let response = client
        ...
        .header("HTTP-Referer", "https://github.com/profiluefter/KMarkdownify")
...
fn main() -> Result<()> {
    ...
    notif.show_loading("KMarkdownify", "Converting PDF to Markdown...\nThis may take a moment.");
    ...
    match convert_pdf(pdf_path, &config, &notif) {
        Ok(markdown_content) => {
            ...
            notif.success(&format!("✓ Conversion complete!\n\nSaved to: {:?}", output_path));
            Ok(())
        }
        Err(e) => {
